...
public sealed class SkiaRenderContext : IRenderContext, IDisposable
{
    ...
    private static void ApplyLineStyle(SKPaint paint, LineStyle style)
    {
        ...
        {
            LineStyle.Dash => SKPathEffect.CreateDash([8f, 4f], 0),
            LineStyle.Dot => SKPathEffect.CreateDash([2f, 4f], 0),
            LineStyle.DashDot => SKPathEffect.CreateDash([8f, 4f, 2f, 4f], 0),
            LineStyle.DashDotDot => SKPathEffect.CreateDash([8f, 4f, 2f, 4f, 2f, 4f], 0),
            _ => null
        };
    }
    ...
    /// <inheritdoc />
    public void SetClipPath(IReadOnlyList<(double[] X, double[] Y)> contours)
    {
        if (contours == null || contours.Count == 0) return;

        using var path = new SKPath { FillType = SKPathFillType.Winding }; // non-zero: overlaps union
        var any = false;
        foreach (var (xs, ys) in contours)
        {
            if (xs == null || ys == null || xs.Length < 3 || xs.Length != ys.Length) continue;
        ...
        if (any) _canvas.ClipPath(path, antialias: true);
    }
    ...
    /// <inheritdoc />
    public void BeginGroup(string id) => SaveState(); // raster has no addressable groups; just save state

    /// <inheritdoc />
    public void EndGroup() => RestoreState();
